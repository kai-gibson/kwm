...
/* array of pointers to functions that take an Xevent* as argument */
...
applyrules(Client *c)
{
	...
	x_class    = ch.res_class ? ch.res_class : broken;
	instance = ch.res_name  ? ch.res_name  : broken;

	for (i = 0; i < LENGTH(rules); i++) {
		r = &rules[i];
		if ((!r->title || strstr(c->name, r->title))
		&& (!r->x_class || strstr(x_class, r->x_class))
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		do
			x += TEXTW(tags[i]);
		while (ev->x >= (int)x && ++i < LENGTH(tags));
		if (i < LENGTH(tags)) {
			click = ClkTagBar;
			arg.ui = 1 << i;
...
//	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
	...
	for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)//		mw = m->ww;
		if ((int)i < m->nmaster) {
			//h = (m->wh - my) / (MIN((int)n, m->nmaster) - i);
			//resize(c, m->wx, m->wy + my, mw - (2*c->bw), h - (2*c->bw), 0);
			//if ((int)(my + HEIGHT(c)) < m->wh)
			//	my += HEIGHT(c);
            r = MIN((int)n, m->nmaster) - i;
            h = (m->wh - my - m->gappoh*oe - m->gappih*ie * (r - 1)) / r;
            resize(c, m->wx + m->gappov*oe, m->wy + my, mw - (2*c->bw) - m->gappiv*ie, h - (2*c->bw), 0);
            if ((int)(my + HEIGHT(c) + m->gappih*ie) < m->wh)
			...
			if ((int)(ty + HEIGHT(c) + m->gappih*ie) < m->wh)
...
systraytomon(Monitor *m) {
	...
	for(i = 1, t = mons; t && t->next && i < (int)systraypinning; i++, t = t->next) ;
	if(systraypinningfailfirst && n < (int)systraypinning)
		return mons;
	return t;
}
